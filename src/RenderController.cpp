...
RenderController::RenderController
    ...
    // signal for check box for showing vertices
    QObject::connect(   renderWindow->LoopSubdivisionButton, 		        SIGNAL(clicked()),
                        this,                                       SLOT(btnLoopDivision()));

    // signal for check box for showing vertices
    QObject::connect(   renderWindow->WriteFileButton, 		        SIGNAL(clicked()),
                        this,                                       SLOT(btnWriteFile()));
...
void RenderController::btnLoopDivision()
{
    renderWindow->ResetInterface();

    directedEdgeSurface->loop_subdivision();
}

void RenderController::btnWriteFile()
{
    renderWindow->ResetInterface();


    directedEdgeSurface->Write2System();
}
